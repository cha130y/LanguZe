...
1. **Given** a `READY` world with no unfinished game, **when** I open it, **then** I see its photo and its words, each with its Thai meaning, CEFR level, and my mastery level.
2. **Given** a `READY` world, **when** I open it, **then** I can start an Identify game (US-030) and remove wrong words (US-022).
3. **Given** a game for that world that I have not finished, **when** I open the world, **then** the word list is hidden, and I am told that finishing the game brings it back (S8).
4. **Given** an `ANALYZING` or `FAILED` world, **when** I open it, **then** I see its status and, for `FAILED`, what I can do next (US-021).
5. **Given** a world that belongs to another learner, **when** I try to open it, **then** it is refused.
...
Writing acceptance criteria exposed points the SRS did not settle. S1 to S7 were decided on 2026-09-19 and recorded in SRS v0.6; later rows carry their own date.
...
| S8  | The world page shows every word with its meaning, so a learner can read the answers while a game is running. Should the game hide them?                                         | Yes, but only while a game for that world is unfinished. Studying before playing is the point of US-012; reading the answers mid-game is not, and mastery has to mean "I recalled this" for review (FR-050), the tutor (FR-072), and progress (FR-060) to be worth anything. Removing a word (FR-026) waits until the game is finished, which is the cost of the simpler rule. Difficulty levels that show more or less help were considered and deferred: help that does not change XP or mastery would let a learner bank a mastery they have not earned, so it needs a design of its own, after review and progress exist. | FR-014, FR-026, US-012 | Confirmed 2026-09-25 |
